 ...
 */
public class Demo06RecursionOptimizeCoin {
    ...
    int getMinCountsHelper(int total, int[] values) {
        ...
        for (int i = 0; i < valueLength; i++) {
            // 遍历所有面值
            int currentValue = values[i];         // 如果当前面值大于硬币总额,那么跳过
            if (currentValue > total) {
                continue;
            }
            int rest = total - currentValue; // 使用当前面值,得到剩余硬币总额
            int restCount = getMinCountsHelper(rest, values);         // 如果返回-1,说明组合不可信,跳过
            if (restCount == -1) {
                continue;
            }
    ...
    int getMinCountOfCoinsAdvance() {
        ...
        // 硬币面值的数组
        int total = 11; // 总值
        return getMinCountsHelper(total, values);
        // 输出答案
    }
}
