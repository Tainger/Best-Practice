 ...
 */
public class Demo08BeiWangLuFindCoin {
    ...
    public int getMinCountsSol() { // 入口函数
        int[] values = {1, 2, 5}; // 硬币面值
        int total = 11; // 总值
        int[] memo = new int[total + 1];     // , -2); // 备忘录,没有缓存的元素为-2
        Arrays.fill(memo, -2);
        memo[0] = 0; // 其中0对应的结果也是0,首先存在备忘录中   // 求得最小的硬币数量,并输出结果
        return getMinCountsHelper(total, values, memo); // 输出结果
    }

    public static void main(String[] args) {
//        System.out.println(getMinCountsSol());
    }
}
